class HappyMeals {
  ...
  meals() {
    ...
    let sectionMeal = document.getElementById('carousel');

    // for each day in week
    for (var day in this.weekMap) {

      // create a div with text = day 
      ...
      let daysMeal = this.weekMap[day];
      //console.log(day, daysMeal);

      // for each meal in day
      for (const meals in daysMeal) {

        if (meals == 0) {
          let mealName = document.createElement('h4');
          mealName.innerText = "Petit déjeuner";
          div.append(mealName);
        }
        ...
        if (meals == 3) {
          //console.log('diner');
          let mealName = document.createElement('h4');
          mealName.innerText = "Dinner";
          div.append(mealName);
        }


        let arrayOfFood = daysMeal[meals]

        // for each food in meal
        arrayOfFood.forEach(function (foodInfos) {

          // for each info in food
          for (const foodInfo in foodInfos) {
            //console.log(`${foodInfo}: ${foodInfos[foodInfo]}`);
            ...
            if (foodInfo == "name") {
              ...
              foodName.innerText = `${foodInfos[foodInfo]}`;
              div.appendChild(foodName);

            }

            if (foodInfo == "portions") {

              let foodPortions = document.createElement('div');
              foodPortions.classList.add("portion");
              let portionSpan = document.createElement('span');
              portionSpan.classList.add("badge");
              portionSpan.innerText = `${foodInfos[foodInfo]}`;
              foodPortions.appendChild(portionSpan);
              div.appendChild(foodPortions);
